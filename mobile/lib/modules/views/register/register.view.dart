...
class _CreateAccountViewState extends State<CreateAccountView> {
  ...
  Rx<TextEditingController> profileName =
      Rx<TextEditingController>(TextEditingController());
  Rx<TextEditingController> firstName =
      Rx<TextEditingController>(TextEditingController());
  Rx<TextEditingController> lastName =
      Rx<TextEditingController>(TextEditingController());
  Rx<TextEditingController> phone =
      Rx<TextEditingController>(TextEditingController());
  ...
  Widget build(BuildContext context) {
    if (!widget.isCreate) {
      ...
      profileName.value = TextEditingController(
          text: profileController.profileData['profile_name']);
      firstName.value = TextEditingController(
          text: profileController.profileData['first_name']);
      lastName.value = TextEditingController(
          text: profileController.profileData['last_name']);
      phone.value =
          TextEditingController(text: profileController.profileData['phone']);
      email.value = TextEditingController(
          text: profileController.profileData['user']['email']);
    }
            ...
            child: Column(
              ...
              children: [
                ...
                RimpaTextFormField(
                  ...
                  validator: MultiValidator([
                    RequiredValidator(errorText: ''),
                    PatternValidator(r'^(0[689]{1})+([0-9]{8})$',
                      ...
                      onTap: () async {
                        DateTime? pickedDate =
                            await DatePicker.showSimpleDatePicker(context,
                                ...
                                locale:
                                    DateTimePickerLocale.th, // ✅ ตั้งค่าภาษาไทย
                                looping: false,
                      ...
                      InputDecorator(
                        ...
                        child: Obx(
                          () => AppDropdownV2Component(
                              defaultText: widget.isCreate
                                  ? 'ไม่ระบุ'
                                  : genderMapping[gender.value].toString(),
                              choices: const ['ชาย', 'หญิง', 'ไม่ระบุ'],
                              selected: widget.isCreate
                              ...
                              onchanged: (value) {
                                if (widget.isCreate) {
                                  ...
                                  gender.value =
                                      reverseGenderMapping[value].toString();
                                }
                              }),
                        ),
                      ),
                ...
                if (widget.isCreate)
                  RimpaTextFormField(
                    ...
                    validator: (val) {
                      return MatchValidator(errorText: 'รหัสผ่านไม่ตรงกัน')
